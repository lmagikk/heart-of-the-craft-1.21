...
public class OreGenBlockMenu extends AbstractContainerMenu {
    public final OreGenBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public OreGenBlockMenu(int containerId, Inventory inv, FriendlyByteBuf byteBuf) {
        this(containerId, inv, inv.player.level().getBlockEntity(byteBuf.readBlockPos()), new SimpleContainerData(2));

    }

    public OreGenBlockMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypes.ORE_GEN_BLOCK_MENU.get(), containerId);
        blockEntity = ((OreGenBlockEntity) entity);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.ORE_GENERATOR_BLOCK.get());
    }
